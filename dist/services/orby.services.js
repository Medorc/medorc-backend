...
const getPatientWhereClause = (shc_code, qr_code) => {
    if (shc_code)
...
const getPatientId = async (shc_code, qr_code) => {
...
export const findPatientHospitalVisit = async (entities, shc_code, qr_code) => {
    const whereClause = getPatientWhereClause(shc_code, qr_code);
    if (!whereClause)
        return "Please log in or provide your SHC Code to view hospital visits.";
...
export const findPatientLastRecord = async (shc_code, qr_code) => {
...
export const findPatientLastHospitalVisit = async (shc_code, qr_code) => {
    const patientId = await getPatientId(shc_code, qr_code);
    if (!patientId)
        return "Patient not found.";
...
export const findPatientLastSurgery = async (shc_code, qr_code) => {
    ...
    if (!surgery)
        return "No surgery records were found.";
...
export const checkPatientAllergy = async (shc_code, qr_code) => {
    const patientId = await getPatientId(shc_code, qr_code);
    if (!patientId)
        return "I couldn't find a registered patient matching your account or SHC Code. Please make sure you are logged in or provide your SHC Code.";
    const patient = await prisma.patients.findUnique({
        where: { patient_id: patientId },
        select: { allergy: true }
    });
    if (!patient)
        return "I couldn't find a registered patient matching your account or SHC Code.";
...
export const checkPatientHabits = async (shc_code, qr_code) => {
    const patientId = await getPatientId(shc_code, qr_code);
    if (!patientId)
        return "Patient not found.";
    const patient = await prisma.patients.findUnique({
    ...
    if (patient.alcoholism)
        habits.push("alcoholism");
    if (patient.tobacco)
        habits.push("tobacco use");
    if (patient.others)
        habits.push(patient.others);
    if (habits.length > 0) {
...
export const checkPatientPregnancy = async (shc_code, qr_code) => {
    const patientId = await getPatientId(shc_code, qr_code);
    if (!patientId)
        return "Patient not found.";
    const patient = await prisma.patients.findUnique({
...
export const getPatientOverview = async (shc_code, qr_code) => {
    const patientId = await getPatientId(shc_code, qr_code);
    if (!patientId) {
...
export const getPatientContactInfo = async (shc_code, qr_code) => {
    const patientId = await getPatientId(shc_code, qr_code);
    if (!patientId)
        return "Patient not found.";
...
export const findPatientPastDiagnoses = async (shc_code, qr_code) => {
    const patientId = await getPatientId(shc_code, qr_code);
    if (!patientId)
        return "Patient not found.";
...
export const findPatientCurrentMedications = async (shc_code, qr_code) => {
    const patientId = await getPatientId(shc_code, qr_code);
    if (!patientId)
        return "Patient not found.";
...
export const findPatientDoctorVisit = async (entities, shc_code, qr_code) => {
    const patientId = await getPatientId(shc_code, qr_code);
    if (!patientId)
        return "Patient not found.";
...
export const findPatientLabResults = async (entities, shc_code, qr_code) => {
    const patientId = await getPatientId(shc_code, qr_code);
    if (!patientId) {
        return "Patient not found.";
    }
...
export const findPatientEmergencyContact = async (entities, shc_code, qr_code) => {
    const patientId = await getPatientId(shc_code, qr_code);
    if (!patientId)
        return "Patient not found.";
    const whereFilter = { patient_id: patientId };
...
export const getHealthTip = async (entities) => {
    const whereFilter = {};
...
export const findSpecialistDoctor = async (entities) => {
    const whereFilter = {};
...
export const findHospital = async (entities) => {
    const whereFilter = {};
...
export const getRecordCount = async (entities, shc_code, qr_code) => {
    const patientId = await getPatientId(shc_code, qr_code);
    if (!patientId)
        return "Patient not found.";
    const whereFilter = { patient_id: patientId };
    ...
    if (hospitalEntity)
        whereFilter.hospital_name = { contains: hospitalEntity.value, mode: 'insensitive' };
    if (doctorEntity)
        whereFilter.doctor_name = { contains: doctorEntity.value, mode: 'insensitive' };
    if (diagnosisEntity)
        whereFilter.diagnosis_name = { contains: diagnosisEntity.value, mode: 'insensitive' };
    if (entryTypeEntity)
        whereFilter.entry_type = { equals: entryTypeEntity.value, mode: 'insensitive' };
...
export const findTreatmentsForDiagnosis = async (entities, shc_code, qr_code) => {
    const patientId = await getPatientId(shc_code, qr_code);
    if (!patientId)
        return "Patient not found.";
    const diagnosisEntity = entities.find(e => e.entity === 'diagnosis_name');
    if (!diagnosisEntity)
        return "Please specify a diagnosis.";
    const records = await prisma.patient_medical_records.findMany({
        where: {
            patient_id: patientId,
...
//# sourceMappingURL=orby.services.js.map