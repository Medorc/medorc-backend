...
export const handleGetDoctorProfile = async (req, res) => {
    try {
        ...
        if (userPayload.role == "doctor") {
            ...
            if (!userId) {
                return res.status(400).json({ error: "Doctor id must be provided" });
            }
...
export const handleGetDoctorProfileCredentials = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
...
export const handleGetDoctorBasicDetails = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
...
export const handleUpdateDoctorProfileCredentials = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
        ...
        const userId = String(userPayload.id);
        const newcredentials = req.body.newCredentials;
        if (!newcredentials) {
            res.status(400).json({ error: "Credentials not provided correctly" });
        }
...
export const handleUpdateDoctorVerificationDocument = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
        ...
        if (!newDocument) {
            res.status(400).json({ error: "Document url not provided correctly" });
        }
...
export const handleUpdateDoctorPhoto = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
        ...
        if (!newPhoto) {
            res.status(400).json({ error: "Photo url not provided correctly" });
        }
...
export const handleUpdateDoctorEmail = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
        ...
        if (!newEmail) {
            res.status(400).json({ error: "Email not provided correctly" });
        }
...
export const handleUpdateDoctorPhone = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
        ...
        if (!newPhone) {
            res.status(400).json({ error: "Phone not provided correctly" });
        }
...
export const handleUpdateDoctorPassword = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
        ...
        if (!newPassword) {
            res.status(400).json({ error: "Password not provided correctly" });
        }
