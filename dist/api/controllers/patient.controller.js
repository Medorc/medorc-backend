...
import {} from "express";
export const handleGetPatientProfile = async (req, res) => {
    try {
        ...
        if (userPayload.role === "patient") {
        ...
        else if (["doctor", "hospital", "extern"].includes(userPayload.role)) {
            ...
            patientProfile = await patientService.getPatientProfile(undefined, shc_code, qr_code);
        }
        else {
        ...
        if (patientProfile) {
            try {
                const visitorId = userPayload.id;
                const visitorRole = userPayload.role;
...
export const handleGetPatientPersonalDetails = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
        ...
        if (userPayload.role === "patient") {
            ...
            patientPersonalDetails = await patientService.getPatientPersonalDetails(undefined, shc_code, qr_code);
        }
        else {
            return res.status(403).json({ error: "Unauthorized role." });
        }
...
export const handleGetPatientBasicDetails = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
        ...
        if (userPayload.role === "patient") {
            ...
            patientBasicDetails = await patientService.getPatientBasicDetails(undefined, shc_code, qr_code);
        }
        else {
            return res.status(403).json({ error: "Unauthorized role." });
        }
...
export const handleGetPatientEmergencyContacts = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
        ...
        if (userPayload.role === "patient") {
            ...
            if (shc_code)
                patientIdentifier.shc_code = shc_code;
            if (qr_code)
                patientIdentifier.qr_code = qr_code;
        }
        else {
            return res.status(403).json({ error: "Unauthorized role." });
        }
...
export const handleGetPatientDataLogs = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
...
export const handleUpdatePatientVisibility = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
...
export const handleUpdatePatientPhoto = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
...
export const handleUpdatePatientLifestyle = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
        ...
        let newLifestyle = req.body.newLifestyle;
...
export const handleUpdatePatientPersonalDetails = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
...
export const handleUpdatePatientEmail = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
...
export const handleUpdatePatientPhoneNo = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
...
export const handleUpdatePatientPassword = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
...
export const handleAddPatientEmergencyContact = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
        }
        let emergencyContact = req.body.newEmergencyContact;
...
export const handleDeletePatientEmergencyContact = async (req, res) => {
    try {
        const userPayload = req.user;
        if (!userPayload || typeof userPayload !== "object") {
            return res.status(400).json({ error: "Invalid token payload." });
        }
        const emg_id = req.body.emg_id;
        if (!emg_id) {
...
export const handleCreatePatientRecord = async (req, res) => {
    try {
        const userPayload = req.user;
        const { basicDetails, hospitalizationDetails, surgeryDetails, documents, shc_code, // Doctor/Hospital will provide this in the body
        qr_code // Or this
         } = req.body;
        ...
        else if (['doctor', 'hospital'].includes(userPayload.role)) {
            if (!shc_code && !qr_code) {
                return res.status(400).json({
        ...
        const newRecord = await patientService.createPatientRecord(patientIdentifier, {
            basicDetails,
...
export const handleAddPatientHospitalizationDetails = async (req, res) => {
    try {
        const userPayload = req.user;
...
export const handleAddPatientSurgeryDetails = async (req, res) => {
    try {
        const userPayload = req.user;
...
export const handleAddPatientPrescription = async (req, res) => {
    try {
        const userPayload = req.user;
        const record_id = req.params.record_id;
...
export const handleRemovePatientPrescription = async (req, res) => {
    try {
        const userPayload = req.user;
        const record_id = req.params.record_id;
...
export const handleAddPatientLabResults = async (req, res) => {
    try {
        const userPayload = req.user;
        const record_id = req.params.record_id;
...
export const handleRemovePatientLabResults = async (req, res) => {
    try {
        const userPayload = req.user;
        const record_id = req.params.record_id;
        const { shc_code, qr_code } = req.body;
...
export const handleUpdatePatientRecordVisibility = async (req, res) => {
    try {
...
export const handleGetPatientRecords = async (req, res) => {
    try {
        const userPayload = req.user;
...
export const handleGetPatientSurgeryDetails = async (req, res) => {
    try {
        const record_id = req.params.record_id;
        if (!record_id) {
...
export const handleGetPatientHospitalizationDetails = async (req, res) => {
    try {
        const record_id = req.params.record_id;
        if (!record_id) {
            return res.status(403).json({ error: 'record Id must be provided' });
...
export const handleGetPatientDocuments = async (req, res) => {
    try {
        const record_id = req.params.record_id;
        if (!record_id) {
            return res.status(403).json({ error: 'record Id must be provided' });
